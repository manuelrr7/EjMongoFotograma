 ...
 * @version  Ajax-MVC2
 */
public class AccionBFotogramas implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		String json, direc;
		direc= request.getParameter("directores");
		modelo.setContentType("application/json; charset=UTF-8");
		Fotograma fotograma = new Fotograma();
		
		if (procesaBD.darFotogramasDirector(direc) != null){		
			
		
			
		}else{
			fotograma.setError("Error al cargar la lista de fotogramas");
		}
		json = new Gson().toJson(procesaBD.darFotogramasDirector(direc));
